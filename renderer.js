...
function updateDirectionIndicator() {
  ...
  if (angle < -Math.PI * 0.75 || angle > Math.PI * 0.75)        direction = "Backward";
  else if (angle < -Math.PI * 0.25)                             direction = "Left";
  else if (angle < Math.PI * 0.25)                              direction = "Forward";
  else                                                          direction = "Right";

  const newText = `Camera Direction: ${direction}`;
  if (newText !== lastDirectionText) {
    directionIndicator.innerText = newText;
    lastDirectionText = newText;
  }
}
...
function loadScene(file) {
  ...
  reader.onload = () => {
    ...
    if (loadSpeed && settings.speed !== undefined) {
      speed = settings.speed;
    }
    if (loadSpawnMode && settings.spawnMode !== undefined) {
      spawnMode = settings.spawnMode;
    }
    if (loadSpawnPos && settings.spawnAtCamera !== undefined) {
      spawnAtCamera = settings.spawnAtCamera;
      updatePositionModeIndicator();
    }
    if (loadSpinMode && settings.spinDirection !== undefined) {
      spinDirection = settings.spinDirection;
    }
    if (loadBackground && settings.background !== undefined) {
      scene.background = new THREE.Color(settings.background);
      document.getElementById('bgColorPicker').value = settings.background;
    }
    if (loadTextColor && settings.textColor !== undefined) {
    ...
    if (loadShadows && settings.shadowsEnabled !== undefined) {
      document.getElementById('toggleShadows').checked = settings.shadowsEnabled;
      dirLight.visible              = settings.shadowsEnabled;
      renderer.shadowMap.enabled    = settings.shadowsEnabled;
    }

    if (gridHelper) {
        gridHelper.visible = loadGridOption;
        document.getElementById('toggleGrid').checked = loadGridOption;
    }
